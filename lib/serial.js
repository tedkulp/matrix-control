...
module.exports = exports = (pathToPort, label, numInputs, numOutputs) => {
  ...
  parser.on('data', data => {
    // if (label === 'top') console.log(label, data);
    if (data.length === 13) {
      portStatus = data;
    }
  });
  ...
  const getStatus = () => {
    //console.log(`${label} - ${numInputs}x${numOutputs}`, portStatus);
  ...
  const runCmd = (port, cmdGroup) => {
    Promise.map(cmdGroup.split(''), (input, idx) => {
      return new Promise((resolve, reject) => {
        ...
        _.delay(() => {
          var cmdInt = (idx * 4) + (parseInt(input) - 1);
          var cmd = _.pad(cmdInt.toString(16), 2, '0') + _.pad((255 - cmdInt).toString(16), 2, '0') + 'd57b';
          console.log('sending', cmdInt, cmd);
